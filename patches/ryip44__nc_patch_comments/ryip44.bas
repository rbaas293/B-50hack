...
for x = 0 to 3
	if relay_cfg(x)>=1 then ioctl x+1,1 else ioctl x+1,0 ' 20200501 changed to support NC active while calling and comunicating
next x
...
2030
	...
	for x = 0 to 3
		if or(relay_cfg(x)=2,relay_cfg(x)=4) then
			...
			ioctl x+1,0 ' 20200501 changed to support NC active while calling and comunicating
		endif
2031
	next x
	return
	...
	for x = 0 to 3
		...
		if relay_cfg(x)>1 then
			...
			if or(relay_cfg(x)=3,relay_cfg(x)=4) then
				...
				if relay_cfg(x)=2 then ioctl x+1,1 ' 20200501 changed to support NC active while calling and comunicating
			endif
		endif
2041
	next x
...
2060
	...
	for x = 0 to 3
		...
		if relay_cfg(x)>1 then ioctl x+1,1 ' 20200501 changed to support NC active while calling and comunicating
2061
	next x
	return
